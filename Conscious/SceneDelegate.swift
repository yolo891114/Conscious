...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        UserDefaults.standard.publisher(for: \.isPasswordSet)
            .sink { newValue in
                GlobalState.isLock = newValue
            }
    ...
    func sceneWillEnterForeground(_ scene: UIScene) {
        ...
        if Auth.auth().currentUser == nil {
            ...
            if GlobalState.isLock && isPasswordSet {
                if let rootViewController = self.window?.rootViewController {
                    if let enterPasswordViewController = storyboard.instantiateViewController(withIdentifier: "EnterPasswordViewController") as? EnterPasswordViewController {
...
extension UserDefaults {
    @objc dynamic var isPasswordSet: Bool {
